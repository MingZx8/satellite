...
import re
import time
from multiprocessing import Array, Process, Queue, Manager
...
def get_args():
    ...
    parser.add_argument('--model', type=str, default='../models/MnihCNN_multi.py')
    parser.add_argument('--param', type=str, default='../models/epoch-400.model')
    parser.add_argument('--test_sat_dir', type=str, default='/home/ming/Desktop/Satellite/code/ssai-cnn/test')
    ...
    for d in range(0, args.map_size // 2, (args.map_size // 2) // args.offset):
        ...
        for y in range(d, args.h_limit, args.map_size):
            for x in range(d, args.w_limit, args.map_size):
                if (((y + args.sat_size) > args.h_limit) or ((x + args.sat_size) > args.w_limit)):
                    break
                # ortho patch
                o_patch = ortho[y:y + args.sat_size, x:x + args.sat_size, :].astype(np.float32, copy=False)
...
def tile_patches(args, canvas, queue, canvas_d):
    canvas = np.array(canvas, dtype=object)
    for d in range(0, args.map_size // 2, (args.map_size // 2) // args.offset):
        st = time.time()
        for y in range(d, args.h_limit, args.map_size):
            for x in range(d, args.w_limit, args.map_size):
                if (((y + args.sat_size) > args.h_limit) or
                    ...
                    break
                print(d, '/', args.map_size // 2, ' ', y, '/', args.h_limit, ' ', x, '/', args.w_limit)
                pred = queue.get()
                if pred is None:
                    break
                if pred.ndim == 3:
                    pred = pred.transpose((1, 2, 0))
                    canvas[y:y + args.map_size, x:x + args.map_size, :] = canvas[y:y + args.map_size,
                                                                          x:x + args.map_size, :] + pred
                else:
                    canvas[y:y + args.map_size, x:x + args.map_size, 0] = canvas[y:y + args.map_size,
                                                                          x:x + args.map_size, 0] + pred

        print('offset:{} ({} sec)'.format(d, time.time() - st))
        print('canvas prior : ', canvas.shape)
        canvas_d[len(canvas_d)] = canvas
...
def get_predict(args, ortho, model):
    ...
    args.canvas_h = h_num * args.map_size - (args.sat_size - args.map_size) + args.offset - 1
    args.canvas_w = w_num * args.map_size - (args.sat_size - args.map_size) + args.offset - 1
    ...
    patch_worker = Process(
        target=create_minibatch,
        args=(args, ortho, patch_queue))
    canvas_worker = Process(
        target=tile_patches,
        args=(args, canvas, preds_queue, d))
    patch_worker.start()
    canvas_worker.start()
    ...
    canvas /= args.offset
    print('canvas final: ', canvas.shape)
...
if __name__ == '__main__':
    ...
    for fn in glob.glob('{}/*.tif*'.format(args.test_sat_dir)):
        ...
        #             result[i][j][k] = pred[i][j][k].data

        # cv.imshow('win', result)
        # cv.waitKey(0)
        # cv.destroyAllWindows()

        out_fn = '{}/{}.npy'.format(out_dir, os.path.splitext(os.path.basename(fn))[0])
        np.save(out_fn, pred)

        # out_fn = '{}/{}.png'.format(out_dir, os.path.splitext(os.path.basename(fn))[0])
        # cv.imwrite(out_fn, result * 255)
